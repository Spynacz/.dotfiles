...
def stripAds(path_to_file):
    ...
    if os.path.isfile(path_to_file):
        file_path_parts_list = Path(
            str(path_to_file)
        ).parts  # break the path up into an array
        ...
        if do_synced:
            ...
            if os.path.isfile(sync_file_full):
                for line in fileinput.input(
                    sync_file_full, inplace=True, encoding="utf-8"
                ):
                    try:
                        if not re.search(regex_to_find, line, re.IGNORECASE):
                            print(line, end="")
                            sync_modified = (
                                "[[ SYNCED Subtitle --> Nothing to be Modified ]]"
                            )
                        else:
                ...
                sync_modified = "[[ SYNCED Subtitle --> Not found ]"
        return_str = (
            "Remove-Ads-From-Subs: " + standard_modified + " - " + sync_modified
        )
        return return_str
    else:
